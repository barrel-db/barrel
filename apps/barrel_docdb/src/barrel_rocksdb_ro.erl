...
%% @doc Open the default column family only.
-spec open(string(), list()) -> {ok, rocksdb:db_handle()} | {error, term()}.
open(Path, DbOpts) ->
    case check_store(Path) of
        ok -> rocksdb:open_readonly(Path, ro_opts(DbOpts));
        {error, _} = Err -> Err
    end.
