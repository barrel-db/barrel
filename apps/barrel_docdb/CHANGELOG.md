...
- Documentation refresh: the doc set now describes the HLC version-vector model
  (dropping the removed rev-tree functions `put_rev`/`revsdiff`), and the HTTP
  API pages point to `barrel_server` since barrel_docdb is embedded-only. See
  the umbrella [CHANGELOG](../../CHANGELOG.md).
