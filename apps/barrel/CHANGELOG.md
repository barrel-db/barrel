...
- Requires `barrel_docdb ~> 1.7` (observed version, read-only opens), `barrel_vectordb ~> 2.5` (read-only stores, durable disk BM25) and `barrel_embed ~> 2.5` (model identity), and starts `crypto`.
