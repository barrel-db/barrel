...
model_info(#{model := _} = Config) ->
    barrel_embed_provider:model_info_from(Config, model, undefined);
model_info(Config) ->
    %% the deployment names the model on Azure
    barrel_embed_provider:model_info_from(Config, deployment, undefined).
