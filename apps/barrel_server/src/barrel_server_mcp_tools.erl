...
%% then keeps it live. Segments live under the server's data dir. The
%% index is derived data, so a corpus in an older on-disk format (no
%% corpus.meta, an older manifest or segment version) is rebuilt.
ensure_corpus(Name) ->
    case barrel_ngram:is_open(Name) of
        ...
        false ->
            ...
            Opts = #{db => Name, data_dir => DataDir, on_legacy => reindex},
            %% any other failure surfaces as {error, corpus_not_open}
