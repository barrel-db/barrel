...
-export([
    ...
    multipart_gc_sweep_now_zero_aborts_orphaned_upload/1,
    multipart_gc_sweep_now_default_skips_fresh_upload/1,
    multipart_gc_open_registers_target/1,
    multipart_gc_sweep_does_not_touch_foreign_prefix/1,
    multipart_gc_destroy_aborts_own_dangling_upload/1,
    multipart_gc_destroy_deregisters_target/1,
...
-define(CASES, [
    ...
    multipart_gc_sweep_now_zero_aborts_orphaned_upload,
    multipart_gc_sweep_now_default_skips_fresh_upload,
    multipart_gc_open_registers_target,
    multipart_gc_sweep_does_not_touch_foreign_prefix,
    multipart_gc_destroy_aborts_own_dangling_upload,
    multipart_gc_destroy_deregisters_target,
...
%%====================================================================
%% Multipart upload GC (barrel_att_s3_multipart_gc)
%%====================================================================

%% Simulates a crash mid-upload: cross the multipart threshold, then
%% never call finish_stream/abort_stream at all.
multipart_gc_sweep_now_zero_aborts_orphaned_upload(Config) ->
    AttRef = ?config(att_ref, Config),
    DbName = ?config(db_name, Config),
    ...
    ?assert(upload_listed(Client, Bucket, Key, UploadId)),
...
multipart_gc_sweep_now_default_skips_fresh_upload(Config) ->
    ...
    {ok, S2} = barrel_att_s3_store:write_chunk(S1, binary:copy(<<"y">>, ?PART_SIZE)),
    #{key := Key, multipart := #{upload_id := UploadId}} = S2,
    {ok, _} = barrel_att_s3_multipart_gc:sweep_now(),
    ?assert(upload_listed(Client, Bucket, Key, UploadId)),
    %% the one case that doesn't self-clean via the sweep
    ok = livery_s3:abort_multipart_upload(Client, Bucket, Key, UploadId).

%% Proves open/2 itself registers the target, not just that sweep_now
%% works in general: a fresh store on its own path/db_name/prefix, with a
%% stale upload created directly (bypassing put_stream) under that
%% prefix.
multipart_gc_open_registers_target(Config) ->
    S3Opts = ?config(s3_opts, Config),
    Path = ?config(path, Config) ++ "-gc-register",
    DbName = <<"gc-register-", (?config(db_name, Config))/binary>>,
    {ok, AttRef} = barrel_att_s3_store:open(Path,
        #{s3 => S3Opts#{part_size => ?PART_SIZE}, db_name => DbName}),
    #{client := Client, bucket := Bucket, prefix := Prefix} = AttRef,
    Key = <<Prefix/binary, "/gc-register-key">>,
    {ok, UploadId} = livery_s3:create_multipart_upload(
        Client, Bucket, Key, #{content_type => <<"application/octet-stream">>}),
    {ok, _} = barrel_att_s3_multipart_gc:sweep_now(0),
    ?assertNot(upload_listed(Client, Bucket, Key, UploadId)),
    ok = barrel_att_s3_store:close(AttRef).

%% The sweep only touches prefixes some open/2 actually registered --
%% an upload under a prefix nothing opened (standing in for a different
%% tenant/store sharing the bucket) must survive.
multipart_gc_sweep_does_not_touch_foreign_prefix(Config) ->
    AttRef = ?config(att_ref, Config),
    #{client := Client, bucket := Bucket} = AttRef,
    ForeignKey = <<"foreign-prefix-never-opened/some-key">>,
    {ok, UploadId} = livery_s3:create_multipart_upload(
        Client, Bucket, ForeignKey, #{content_type => <<"application/octet-stream">>}),
    {ok, _} = barrel_att_s3_multipart_gc:sweep_now(0),
    ?assert(upload_listed(Client, Bucket, ForeignKey, UploadId)),
    %% the one case that doesn't self-clean via the sweep
    ok = livery_s3:abort_multipart_upload(Client, Bucket, ForeignKey, UploadId).

%% destroy/2 doesn't just clean up completed objects -- it also aborts any
%% multipart upload still dangling under its own prefix immediately,
%% rather than waiting for the next scheduled GC pass.
multipart_gc_destroy_aborts_own_dangling_upload(Config) ->
    AttRef = ?config(att_ref, Config),
    DbName = ?config(db_name, Config),
    #{client := Client, bucket := Bucket} = AttRef,
    {Key, UploadId} = crash_upload(AttRef, DbName, <<"dangling.bin">>),
    ?assert(upload_listed(Client, Bucket, Key, UploadId)),
    ?assertEqual(ok, barrel_att_s3_store:destroy(AttRef, DbName)),
    ?assertNot(upload_listed(Client, Bucket, Key, UploadId)).

%% destroy/2 is a permanent signal -- unlike close/1 (a bucket can still
%% be shared by other open stores), this prefix will never need sweeping
%% again, so it must actually leave the gc's registry, not just get its
%% dangling uploads aborted once.
multipart_gc_destroy_deregisters_target(Config) ->
    AttRef = ?config(att_ref, Config),
    DbName = ?config(db_name, Config),
    #{endpoint := Endpoint, bucket := Bucket, prefix := Prefix} = AttRef,
    ?assertEqual(ok, barrel_att_s3_store:destroy(AttRef, DbName)),
    %% barrier: a sweep_now call from this same process is guaranteed to be
    %% handled after destroy/2's own deregister cast (same sender, same
    %% mailbox order), so the state below is never read too early
    {ok, _} = barrel_att_s3_multipart_gc:sweep_now(0),
    State = sys:get_state(barrel_att_s3_multipart_gc),
    ?assertNot(maps:is_key({Endpoint, Bucket, Prefix}, State)).

%% Two stores, two distinct prefixes, the SAME bucket -- proves the
%% bucket-grouped sweep (one listing shared across every registered
%% prefix for that bucket) still finds and aborts every one of them, not
%% just the first.
multipart_gc_sweeps_multiple_targets_in_one_pass(Config) ->
    S3Opts = ?config(s3_opts, Config),
    BaseDbName = ?config(db_name, Config),
    PathA = ?config(path, Config) ++ "-gc-multi-a",
    PathB = ?config(path, Config) ++ "-gc-multi-b",
    DbNameA = <<"gc-multi-a-", BaseDbName/binary>>,
    DbNameB = <<"gc-multi-b-", BaseDbName/binary>>,
    {ok, AttRefA} = barrel_att_s3_store:open(PathA,
        #{s3 => S3Opts#{part_size => ?PART_SIZE}, db_name => DbNameA}),
    {ok, AttRefB} = barrel_att_s3_store:open(PathB,
        #{s3 => S3Opts#{part_size => ?PART_SIZE}, db_name => DbNameB}),
    #{client := Client, bucket := Bucket} = AttRefA,
    {KeyA, UploadIdA} = crash_upload(AttRefA, DbNameA, <<"a.bin">>),
    {KeyB, UploadIdB} = crash_upload(AttRefB, DbNameB, <<"b.bin">>),
    ?assert(upload_listed(Client, Bucket, KeyA, UploadIdA)),
    ?assert(upload_listed(Client, Bucket, KeyB, UploadIdB)),
    {ok, N} = barrel_att_s3_multipart_gc:sweep_now(0),
    ?assert(N >= 2),
    ?assertNot(upload_listed(Client, Bucket, KeyA, UploadIdA)),
    ?assertNot(upload_listed(Client, Bucket, KeyB, UploadIdB)),
    ok = barrel_att_s3_store:close(AttRefA),
    ok = barrel_att_s3_store:close(AttRefB).

%% A target whose stored client can't actually talk to S3 must not stop
%% OTHER registered targets from being swept -- proves the per-bucket
%% try/catch actually isolates a crash, not just documents the intent.
multipart_gc_survives_bad_target(Config) ->
    AttRef = ?config(att_ref, Config),
    DbName = ?config(db_name, Config),
    #{client := Client, bucket := Bucket} = AttRef,
    {Key, UploadId} = crash_upload(AttRef, DbName, <<"survives.bin">>),
    ok = barrel_att_s3_multipart_gc:register_target(
             <<"http://bogus-endpoint-does-not-exist.invalid">>, Bucket,
             <<"bogus-prefix-for-crash-test">>, not_a_real_client),
    {ok, _} = barrel_att_s3_multipart_gc:sweep_now(0),
    ?assertNot(upload_listed(Client, Bucket, Key, UploadId)),
    ?assert(is_process_alive(whereis(barrel_att_s3_multipart_gc))).

%% The only way to faithfully reproduce a crash during the SUPERVISOR's
%% own init (arm/0 runs inside init/1) is a genuine fresh app boot -- a
%% child restart under an already-running supervisor doesn't exercise the
%% same code path. Safe mid-suite: later tests re-register their own
%% targets via their own init_per_testcase's open/2.
multipart_gc_bad_interval_does_not_crash_app(_Config) ->
    ok = application:stop(barrel_att_s3),
    ok = application:set_env(barrel_att_s3, multipart_gc_interval, -1),
    {ok, _} = application:ensure_all_started(barrel_att_s3),
    ?assertNotEqual(undefined, whereis(barrel_att_s3_multipart_gc)),
    ok = application:unset_env(barrel_att_s3, multipart_gc_interval).

%% Proves the periodic timer path itself works, not just the sweep_now/0,1
%% test hook every other case here uses -- restart the gc child so it
%% re-arms with a short configured interval, then wait for a stale upload
%% to disappear WITHOUT ever calling sweep_now.
multipart_gc_periodic_timer_fires(Config) ->
    AttRef = ?config(att_ref, Config),
    DbName = ?config(db_name, Config),
    #{client := Client, bucket := Bucket, endpoint := Endpoint, prefix := Prefix} = AttRef,
    ok = application:set_env(barrel_att_s3, multipart_gc_interval, 200),
    ok = application:set_env(barrel_att_s3, multipart_gc_max_age, 0),
    ok = supervisor:terminate_child(barrel_att_s3_sup, barrel_att_s3_multipart_gc),
    {ok, _} = supervisor:restart_child(barrel_att_s3_sup, barrel_att_s3_multipart_gc),
    %% the fresh gc process starts with an empty registry -- re-register
    %% this test's own target, same as open/2 would on its next open
    ok = barrel_att_s3_multipart_gc:register_target(Endpoint, Bucket, Prefix, Client),
    {Key, UploadId} = crash_upload(AttRef, DbName, <<"timer.bin">>),
    ?assert(upload_listed(Client, Bucket, Key, UploadId)),
    ok = wait_until(fun() -> not upload_listed(Client, Bucket, Key, UploadId) end, 3000),
    ok = application:unset_env(barrel_att_s3, multipart_gc_interval),
    ok = application:unset_env(barrel_att_s3, multipart_gc_max_age).

crash_upload(AttRef, DbName, AttName) ->
    {ok, S1} = barrel_att_s3_store:put_stream(AttRef, DbName, <<"doc1">>,
                                              AttName, <<"application/octet-stream">>),
    {ok, S2} = barrel_att_s3_store:write_chunk(S1, binary:copy(<<"x">>, ?PART_SIZE)),
    #{key := Key, multipart := #{upload_id := UploadId}} = S2,
    {Key, UploadId}.

upload_listed(Client, Bucket, Key, UploadId) ->
    {ok, #{uploads := Uploads}} = livery_s3:list_multipart_uploads(Client, Bucket, #{}),
    lists:any(fun(#{key := K, upload_id := U}) -> K =:= Key andalso U =:= UploadId end,
              Uploads).
