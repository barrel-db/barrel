...
%%%
%%% == Multipart upload GC ==
%%% `open/2' registers `{Endpoint, Bucket, Prefix}' with
%%% `barrel_att_s3_multipart_gc' (started with the app), a periodic sweeper
%%% that aborts multipart uploads older than `multipart_gc_max_age' --
%%% orphaned by a hard crash mid-upload, which `abort_stream/1' only covers
%%% for a clean in-process failure. `destroy/2' also calls it directly
%%% (age 0) so a deleted database's own dangling uploads don't wait for the
%%% next scheduled pass. See `docs/limitations.md'.
%%%
...
open(Path, Options) ->
    ...
    case maps:find(bucket, S3Opts) of
        ...
        {ok, Bucket} ->
            _ = application:ensure_all_started(barrel_att_s3), %% starts the multipart-gc sweeper too
            _ = application:ensure_all_started(livery_s3),
            Endpoint = maps:get(endpoint, S3Opts),
...
%% just no longer resumed on a future open that will never happen), then
%% aborts this prefix's own dangling multipart uploads immediately rather
%% than waiting for the next scheduled GC pass, and deregisters it -- unlike
%% close/1 (a bucket can still be shared by other open stores), destroy/2 is
%% a permanent signal this prefix will never need sweeping again.
-spec destroy(map(), binary()) -> ok | {error, term()}.
destroy(#{client := Client, bucket := Bucket, prefix := Prefix, path := Path,
          endpoint := Endpoint}, _DbName) ->
    _ = clear_fork_pending(Path),
    _ = barrel_att_s3_multipart_gc:abort_stale(Client, Bucket, Prefix, 0),
    ok = barrel_att_s3_multipart_gc:deregister_target(Endpoint, Bucket, Prefix),
