...
Nobody owns both. Barrel does: this umbrella is the substrate, and the memory
layer sits on top.
...
- Sessions with TTL and handoffs are barrel primitives; a handoff is a shared
  space plus a capability whose possession is the right to accept. Sessions
  ride a general document-TTL primitive (`expires_at` + opt-in sweeper).
...
- SHIPPED (phase 8): see decision log row 6.
...
- Local BQL subset for the synced set; no ANN in the browser: brute-force
  cosine over the synced subset, optional transformers.js embeddings,
  offload heavy or global queries to the server.
- SHIPPED (phases 9a, 9b, 9c): see decision log row 7.
