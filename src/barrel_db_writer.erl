...
init([Name]) ->
  ...
  case init_(Name) of
    {ok, Barrel} ->
      ...
      Barrel1 = Barrel#{updated_seq => StartSeq},
      hooks:run(activate_barrel, [Name, StartSeq, Barrel1]),
        ...
        {ok, #{ rev := Rev } = DI2, DocRev, DocBody} when DI2 =/= DI ->
          ...
          case DocStatus of
            ...
            found ->
              ?STORE:update_doc(
                 BarrelRef,
                 DI2#{ seq => barrel_sequence:encode(Seq) }, DocRev, DocBody, OldSeq, OldDel
                )
          end,
...
terminate(_Reason, #{ name := Name } = State) ->
  ...
  ocp:record('barrel/dbs/active_num', -1),
  hooks:run(deactivate_barrel, [Name, State]),
