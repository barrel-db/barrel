...
%%
%% This module tries to warn Barrel before such situations occur, so
%% that it has a higher chance to avoid running out of memory.

-module(barrel_vm_memory_monitor).
...
-export([
  get_total_memory/0, get_vm_limit/0,
...
get_memory_calculation_strategy() ->
  case application:get_env(barrel, vm_memory_calculation_strategy, rss) of
    allocated -> allocated;
    erlang -> erlang;
