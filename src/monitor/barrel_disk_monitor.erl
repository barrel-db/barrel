...
start_link(Limit) ->
  start_link(Limit, fun alarm_handler:set_alarm/1, fun alarm_handler:clear_alarm/1).

start_link(Limit, AlarmSet, AlarmClear) ->
  gen_server:start_link({local, ?SERVER}, ?MODULE, [Limit, {AlarmSet, AlarmClear}], []).

init([Limit, AlarmFuns]) ->
  Dir = dir(),
  {ok, Retries} = application:get_env(barrel, disk_monitor_failure_retries),
  {ok, Interval} = application:get_env(barrel, disk_monitor_failure_retry_interval),
  State = #state{dir          = Dir,
...
internal_update(State = #state { limit   = Limit,
  ...
  case {Alarmed, NewAlarmed} of
    {false, true} ->
      _ = emit_update_info("insufficient", CurrentFree, Limit),
      AlarmSet({{resource_limit, disk, node()}, []});
    {true, false} ->
      _ = emit_update_info("sufficient", CurrentFree, Limit),
      AlarmClear({resource_limit, disk, node()});
    _ ->
      ok
  end,
...
get_disk_free(Dir, {unix, Sun})
  ...
  parse_free_unix(barrel_lib:os_cmd(Df ++ " -k " ++ Dir));
get_disk_free(Dir, {unix, _}) ->
  Df = os:find_executable("df"),
  parse_free_unix(barrel_lib:os_cmd(Df ++ " -kP " ++ Dir));
get_disk_free(Dir, {win32, _}) ->
  parse_free_win32(barrel_lib:os_cmd("dir /-C /W \"" ++ Dir ++ "\"")).
...
interpret_limit({mem_relative, Relative})
  ...
  round(Relative * barrel_vm_memory_monitor:get_total_memory());
